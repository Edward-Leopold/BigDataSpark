...
2. перейдите в корень клонированного репозитория (```cd BigDataSpark```)
3. в консоли выполнить ```docker-compose up -d```
4. Spark скрипты выполняться автоматически из .sh скрипта
5. подождать пару минут
6. проверить таблицы через DBeaver (порты, юзернеймы и пароли указаны в docker-compose.yml, для postgres использовать порт 5433)
7. завершить работу ```docker-compose down -v```
